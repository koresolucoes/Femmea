...
create index if not exists femmea_treatments_user_status_idx on public.femmea_treatments (user_id, status, created_at desc);
...
create policy femmea_treatments_select_own on public.femmea_treatments for select to authenticated using (auth.uid() = user_id);
drop policy if exists femmea_treatments_insert_own on public.femmea_treatments;
create policy femmea_treatments_insert_own on public.femmea_treatments for insert to authenticated with check (auth.uid() = user_id);
drop policy if exists femmea_treatments_update_own on public.femmea_treatments;
create policy femmea_treatments_update_own on public.femmea_treatments for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists femmea_treatments_delete_own on public.femmea_treatments;
create policy femmea_treatments_delete_own on public.femmea_treatments for delete to authenticated using (auth.uid() = user_id);
...
alter table public.femmea_insemination_journeys
  ...
  add column if not exists current_state text not null default 'preparation' check (current_state in ('preparation','cycle_monitoring','procedure_scheduled','procedure_day','waiting_period','test_due','result')),
  add column if not exists status text not null default 'active' check (status in ('active','paused','postponed','cancelled','completed')),
  add column if not exists outcome text check (outcome is null or outcome in ('positive','negative','inconclusive')),
...
where not exists (select 1 from public.femmea_treatments t where t.user_id = j.user_id and t.status = 'active');

update public.femmea_insemination_journeys j
set treatment_id = (
  select t.id from public.femmea_treatments t
  where t.user_id = j.user_id
  order by (t.status = 'active') desc, t.created_at asc
  limit 1
)
...
create index if not exists femmea_journeys_treatment_attempt_idx on public.femmea_insemination_journeys (treatment_id, attempt_number desc);
create index if not exists femmea_journeys_user_status_idx on public.femmea_insemination_journeys (user_id, status, created_at desc);
...
create index if not exists femmea_journey_events_user_start_idx on public.femmea_journey_events (user_id, starts_at);
create index if not exists femmea_journey_events_journey_start_idx on public.femmea_journey_events (journey_id, starts_at);
...
create policy femmea_journey_events_select_own on public.femmea_journey_events for select to authenticated using (auth.uid() = user_id);
drop policy if exists femmea_journey_events_insert_own on public.femmea_journey_events;
create policy femmea_journey_events_insert_own on public.femmea_journey_events for insert to authenticated
  with check (auth.uid() = user_id and (journey_id is null or exists (select 1 from public.femmea_insemination_journeys j where j.id = journey_id and j.user_id = auth.uid())));
drop policy if exists femmea_journey_events_update_own on public.femmea_journey_events;
create policy femmea_journey_events_update_own on public.femmea_journey_events for update to authenticated using (auth.uid() = user_id)
  with check (auth.uid() = user_id and (journey_id is null or exists (select 1 from public.femmea_insemination_journeys j where j.id = journey_id and j.user_id = auth.uid())));
drop policy if exists femmea_journey_events_delete_own on public.femmea_journey_events;
create policy femmea_journey_events_delete_own on public.femmea_journey_events for delete to authenticated using (auth.uid() = user_id);
...
create index if not exists femmea_reminders_journey_idx on public.femmea_reminders (journey_id, scheduled_for);
