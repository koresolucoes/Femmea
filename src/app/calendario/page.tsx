...
export default async function CalendarPage({ searchParams }: Props) {
  ...
  const registerDate = (value:string|null|undefined,label:string,kind:CalendarEvent["kind"]) => { if (!value) return; const [year,m,day]=value.slice(0,10).split("-").map(Number); if (year===month.year&&m===month.month) events.push({day,label,kind}); };
  const registerTimestamp = (value:string|null|undefined,label:string,kind:CalendarEvent["kind"]) => { if (!value) return; const local=dateInTimeZone(new Date(value),timeZone); if(local.year===month.year&&local.month===month.month) events.push({day:local.day,label,kind}); };
