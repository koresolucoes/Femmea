...
export default async function Home() {
  ...
  return <MobileShell active="today">
    ...
    {journey?.cycle_start_date && currentState !== "result" && <section className="guided-cycle-card">
      ...
      <p>A fase exibida é uma referência calculada com as datas que você registrou. Abra o acompanhamento antes de escolher o que deseja anotar hoje.</p>
      <div className="guided-cycle-actions"><Link href="/inseminacao?stage=cycle_monitoring">Acompanhar meu ciclo</Link><Link className="secondary" href="/sintomas/registrar">Registrar como estou</Link></div>
    </section>}
    ...
    <section className="today-section"><div className="today-section-heading"><h2>Para hoje</h2><Link href="/lembretes">Organizar</Link></div><div className="today-list">{todayScheduled.map((item) => <div className="today-task" key={`${item.kind}-${item.at}-${item.title}`}><span><CalendarIcon /></span><div><strong>{item.title}</strong><small>{item.kind === "event" ? "Evento da jornada" : "Lembrete"}</small></div><b>{formatTime(item.at)}</b></div>)}{!symptomResponse.data && <Link className="today-task" href="/sintomas/registrar"><span><HeartIcon /></span><div><strong>Como você está hoje?</strong><small>Registre sintomas e emocional</small></div><b>registrar</b></Link>}{(currentState === "cycle_monitoring" || currentState === "procedure_scheduled") && !observationResponse.data && <Link className="today-task" href="/inseminacao?stage=cycle_monitoring"><span><SparklesIcon /></span><div><strong>Revisar seu ciclo</strong><small>Veja a fase atual antes de escolher um registro</small></div><b>abrir</b></Link>}</div></section>

    <section className="today-section today-register-card"><div><h2>Quer guardar algo de hoje?</h2><p>Use o botão Registrar para salvar somente o que fizer sentido agora.</p></div><Link href="/sintomas/registrar" aria-label="Registrar como estou"><HeartIcon /></Link></section>
