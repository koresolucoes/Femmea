...
export default async function RemindersPage({ searchParams }: Props) {
  ...
  return (
    <MobileShell>
          ...
          return (
            <div className="reminder-row" key={reminder.id}>
              ...
              <div>
                ...
                <small>{reminderLabels[reminder.reminder_type] ?? "Lembrete"} · {formatted.date} às {formatted.time}</small>
              </div>
              <form action={toggleReminder}>
                <input type="hidden" name="reminderId" value={reminder.id} />
                <input type="hidden" name="enabled" value={String(reminder.enabled)} />
                <button className={`toggle-switch ${reminder.enabled ? "on" : ""}`} aria-label={reminder.enabled ? "Desativar lembrete" : "Ativar lembrete"}>
                  <span />
                </button>
              </form>
            </div>
      ...
      <details className="new-reminder-card">
        ...
        <form action={createReminder}>
          ...
          <label>
            ...
            <select name="type" defaultValue="appointment">
              ...
              <option value="appointment">Consulta ou exame</option>
              <option value="cycle">Ciclo</option>
              <option value="pregnancy_test">Teste de gravidez</option>
              <option value="custom">Outro</option>
            </select>
          </label>
          <div className="two-field-row">
            <label><span>Data</span><input name="date" type="date" required /></label>
            <label><span>Horário</span><input name="time" type="time" defaultValue="09:00" required /></label>
          </div>
