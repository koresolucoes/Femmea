...
export default async function InseminacaoPage({ searchParams }: Props) {
  ...
  const [{ data: journeyData }, { data: profile }] = await Promise.all([
    supabase.from("femmea_insemination_journeys").select("id,user_id,current_stage,current_state,status,outcome,treatment_id,attempt_number,cycle_start_date,procedure_date,pregnancy_test_date,created_at,updated_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(1).maybeSingle(),
  ...
  const cycleDay = normalizeCycleDay(getCycleDay(journey.cycle_start_date, timeZone), cycleLength);
  const phaseInfo = CYCLE_PHASES[getEstimatedCyclePhase(cycleDay, cycleLength)];
  const cycleContext = journey.cycle_start_date ? {
    label: phaseInfo.label,
    dayLabel: cycleDay ? `Dia ${cycleDay} de ${cycleLength}` : "Ciclo registrado",
    note: "Referência calculada com as datas salvas no Femmea.",
  } : null;
  ...
  return <MobileShell active="journey"><FemmeaJourneyHeader /><JourneyTabs active={tab} />
    {tab === "timeline" && <><section className="timeline-intro journey-status-intro story-opening"><span className="stage-kicker">Tentativa {journey.attempt_number ?? 1} · sua história, etapa por etapa</span><h2>{stateMeta?.label ?? JOURNEY_STAGES[currentIndex].title}</h2><p>Cada capítulo abre aqui mesmo para manter contexto, informação e ferramentas no mesmo fluxo.</p></section><JourneyTimeline initialStage={initialStage} currentStage={computedStage} procedureDate={formatJourneyDate(journey.procedure_date)} pregnancyTestDate={formatJourneyDate(journey.pregnancy_test_date)} cycleContext={cycleContext} />{(computedStage === "pregnancy_test" || journey.outcome) && <JourneyOutcomeCard journeyId={journey.id} outcome={journey.outcome} />}<details className="journey-settings-card story-settings-card"><summary>Ajustar datas da jornada</summary><form action={updateJourneyDates} className="journey-date-form"><input type="hidden" name="journeyId" value={journey.id} /><label><span>Data do procedimento</span><input name="procedureDate" type="date" defaultValue={procedureInput} /></label><label><span>Data registrada para o teste</span><input name="pregnancyTestDate" type="date" defaultValue={journey.pregnancy_test_date ?? ""} /></label><button type="submit">Salvar datas</button></form></details></>}
